...
df=pd.read_csv("/home/arindam/Desktop/data.csv")
...
def stochastic_grad_des():
    ...
    for e in range(epochs):
        ...
        for k in range(n):
            r=(1/(1+pow(math.e,-np.matmul(inputx[i],w))))
            err+=(1/n)*(inputy[i][0][1]*math.log(r[0][1])+((1-inputy[i][0][1])*math.log(1-r[0][1])))
            w-=learn_rate*((np.matmul(inputx[k].T,((1/(1+pow(math.e,-np.matmul(inputx[k],w))))))-inputy[k])+((lamda/n)*w))
